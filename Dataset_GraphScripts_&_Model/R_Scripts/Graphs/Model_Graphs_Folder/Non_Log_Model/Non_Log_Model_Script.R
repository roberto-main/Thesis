...
df <- read_excel("Final_Dataset_Updated.xlsx", sheet="Countries")
...
# Filtraggio e preparazione dati
df <- df[df$Year < 2025,]
...
residuals_val <- residuals(ayo5b)
fitted_val    <- fitted(ayo5b)
...
# Parametri per la linea QQ
qq_intercept <- mean(residuals_val)
qq_slope     <- sd(residuals_val)

# Grafici neri
p1 <- ggplot(diag_ayo5b, aes(x=fitted_val, y=residuals_val)) +
  geom_point() +
  labs(title="Residuals vs Fitted", x="Fitted", y="Residuals")

p2 <- ggplot(diag_ayo5b, aes(x=residuals_val)) +
  geom_histogram(bins=30) +
  labs(title="Histogram of Residuals", x="Residuals")

p3 <- ggplot(diag_ayo5b, aes(x=theoretical, y=residuals_val)) +
  geom_point() +
  geom_abline(intercept=qq_intercept, slope=qq_slope) +
  labs(title="Q-Q Plot", x="Theoretical Quantiles", y="Sample Quantiles")

p4 <- ggplot(diag_ayo5b, aes(x=fitted_val, y=sqrt_res)) +
  geom_point() +
  labs(title="Scale-Location", x="Fitted", y="sqrt|Residuals|")

# Grafici colorati
p1c <- ggplot(diag_ayo5b, aes(x=fitted_val, y=residuals_val, color=Continent)) +
  geom_point(alpha=0.6) +
  scale_color_manual(values=my_colors) +
  labs(title="Residuals vs Fitted", x="Fitted", y="Residuals")

p2c <- ggplot(diag_ayo5b, aes(x=residuals_val)) +
  geom_histogram(fill=my_colors["EU - West"], bins=30) +
  labs(title="Histogram of Residuals", x="Residuals")

p3c <- ggplot(diag_ayo5b, aes(x=theoretical, y=residuals_val, color=Continent)) +
  geom_point(alpha=0.6) +
  geom_abline(intercept=qq_intercept, slope=qq_slope, color="black") +
  scale_color_manual(values=my_colors) +
  labs(title="Q-Q Plot", x="Theoretical Quantiles", y="Sample Quantiles")

p4c <- ggplot(diag_ayo5b, aes(x=fitted_val, y=sqrt_res, color=Continent)) +
  geom_point(alpha=0.6) +
  scale_color_manual(values=my_colors) +
  labs(title="Scale-Location", x="Fitted", y="sqrt|Residuals|")
...
ggsave("Non_Log_Model_Graph.png", plot=g, width=12, height=10, dpi=300)
ggsave("Non_Log_Model_Colored_Graph.png", plot=g2, width=14, height=10, dpi=300)
