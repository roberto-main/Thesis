...
df %>% #con sales in millions
  ggplot(aes(x = Internet_Users, y = Sales_in_Millions)) +
  geom_point(aes(color = Continent), alpha = 0.6, size = 2) +
  geom_smooth(method = "lm", color = "maroon", se = FALSE) +
  labs(
    title = "Internet Penetration and Video Game Sales",
    x = "Internet Users (%)",
...
ggsave(filename = "LOG_Colored_Internet_Penetration_&_VideoGames_Sales_Graph.png",
