...
df %>% #Con sales in millions
  ggplot(aes(x = Stringency_Index, y = Sales_in_Millions)) +
  geom_point(aes(color = Continent), alpha = 0.6, size = 2) +
  geom_smooth(method = "lm", color = "maroon", se = FALSE) + 
  labs(
    title = "COVID Restrictions and Video Game Sales",
...
# Salvataggio
ggsave(filename = "LOG_Colored_Covid_Restrictions_&_VideoGames_Sales_Graph.png",
